    ...
    newForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            if (response.ok) {
                ...
                alert('TRANSMISSION FAILED: ' + (errorData.errors ? errorData.errors.map(e => e.message).join(', ') : 'Unknown Error'));
                btn.innerText = originalText;
                btn.disabled = false;
            }
        } catch (error) {
...
function initCubeGame() {
    ...
    // Ground
    const levelMap = [];
    for(let i=0; i<10; i++) levelMap.push({x: 20 + i*40, y: 340, type: 'GRASS'});
    // Platforms
    ...
    for(let i=levelMap.length; i<27; i++) {
         levelMap.push({x: Math.random()*380 + 10, y: Math.random()*100 - 150, type: 'CLOUD'}); // Off screen or high up
    }
        ...
        function animateExit() {
            ...
            for (let i = 0; i < particles.length; i++) {
                ...
                if (p.life > 0) {
                    ...
                    ctx.fillStyle = p.color;
                    ctx.fillRect(p.x, p.y, TRANSITION_PFX.pixelSize, TRANSITION_PFX.pixelSize);
                }
            }
